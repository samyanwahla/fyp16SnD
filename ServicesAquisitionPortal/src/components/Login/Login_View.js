...
const mapDispatchToProps = dispatch => {
	return {
		handleSignIn: (username, password) => {
			dispatch(loginServer.handleSignIn(username, password));
		},
		handleBackClick: () => {
			dispatch({ type: login_Actions.login_SignIn.NEW });
		},
	};
};
...
class LoginView extends Component {
	getScreen(status) {
		...
		switch (status) {
			case login_Status.login_SignIn.NEW:
				return <Login_signIn handleSignIn={this.props.handleSignIn} />;
				break;
			...
			case login_Status.login_SignIn.AUTHORIZED:
				...
				return <LoginSuccess handleBackClick={this.props.handleBackClick} />;
				break;
			case login_Status.login_SignIn.NOT_AUTHORIZED:
				return <Login_signIn status={status} handleSignIn={this.props.handleSignIn} />;
				break;
	...
	render() {
		return <div>{this.getScreen(this.props.login_status)}</div>;
	}
}
